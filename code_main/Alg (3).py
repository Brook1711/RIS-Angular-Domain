...
plt.plot(x_test, np.array(prime_y_cal), color = 'red')
plt.axhline(0)
plt.axhline(145.14633243598493)
plt.axvline(0.140626)
...
def f_m(YK = YK, Mu = E_step_instance.Mu, kappa = 100, Omega = Omega_true_L ,Delta_varphi = Delta_varphi, Delta_varphi_m = 0, m = 0):
    ...
    for l in range(L):
        F_kl = Phi_H * generate_V_wl(M, Omega[l]) * DM
        for k in range(K):
            x_k = Mu[k]
            for t in range(tau):
                y_klt = YK[k][t+l*tau][0,0]
...
# # before update $\omega_l$

# In[28]:


x = np.linspace(-0.5 + 0.00001, 0.5 - 0.00001, 200)
Big_az = plt.figure(figsize=(16,16),dpi = 300)
axes = []
for l in range(L):
...
# # Update   $ \ \omega_l \quad \forall l \in \mathcal{L}$
...
def update_omega(l = l, m = m):
    ...
    while True and (cnt < cnt_max):
        ...
        if (x_t_plus_1 > 1/M + Omega_grid[m]) or (x_t_plus_1 < 0 + Omega_grid[m]):
            alpha_step *= tau_para
            continue
...
Delta_Omega_container_max_index = np.zeros(L)
for l in range(L):
    l_max = -np.inf
    for m in range(M):
        Delta_Omega_container[l,m] = (1/M) * 0.5
        l_value = update_omega(l,m)
        if l_value > l_max:
            l_max = l_value
            Delta_Omega_container_max_index[l] = m
...
for l in range(L):
    ...
    m = int(Delta_Omega_container_max_index[l])
    plt.scatter(
        Delta_Omega_container[l,m] +Omega_grid[m], 
        f_l(
            YK=YK, 
            Mu = E_step_instance.Mu, 
            kappa=0.001,
            omega_l= Delta_Omega_container[l,m] + Omega_grid[m],
            l = l, 
            Delta_varphi= Delta_varphi_container
        ),
        color = 'red',
    ...
    Omega_container[l] = Delta_Omega_container[l,m] + Omega_grid[m]
...
M_varphi_plots = []
x_list = []
Delta_x_list = np.linspace(0 + 0.00001, 1/M - 0.00001, 10)
plt.figure(figsize=(16,4),dpi = 300)

varphi_offset = np.array([m/M - 0.5 for m in range(M)])
for m in range(M):
    for Delta_x in Delta_x_list:
        varphi_now = varphi_offset[m] + Delta_x 
        x_list.append(varphi_now)
        M_varphi_plots.append(
            f_m(
                YK = YK, 
                Mu = E_step_instance.Mu, 
                kappa=0.001, 
                Omega = Omega_container,
                Delta_varphi= Delta_varphi_container,
                Delta_varphi_m=Delta_x, 
                m = m
            )
        )
    plt.axvline(Omega_grid[m], linestyle = '--')
plt.plot(x_list, M_varphi_plots)

sca_y = []
sca_x = Delta_varphi_container + varphi_offset
for m in range(M):
    sca_y.append(
        f_m(
            YK = YK, 
                ...
                Omega = Omega_container,
                Delta_varphi= Delta_varphi_container,
                Delta_varphi_m=Delta_varphi_container[m], 
                m = m
        )
    )
plt.scatter(
...
def update_varphi(m = m):
    ...
    while True and (cnt < cnt_max):
        ...
        x_t_plus_1 = x_t + alpha_step * np.sign(m_gri)
        if (x_t_plus_1 > 1/M) or (x_t_plus_1 < 0):
            alpha_step *= tau_para
            continue
        # print(x_t, x_t_plus_1)
        f_t = f_m(
        ...
        f_t_plus_1 = f_m(
            ...
            Omega=Omega_container, 
            Delta_varphi = Delta_varphi_container,
            Delta_varphi_m= x_t_plus_1,
            m=m
        )
        # print(alpha_step)
        # print("ft+1 - ft",f_t_plus_1 - f_t, np.abs(c_para * m_gri * alpha_step))
        if f_t_plus_1 >= f_t + np.abs(c_para * m_gri * alpha_step):
            Delta_varphi_container[m] += alpha_step * np.sign(m_gri)
            # print(alpha_step,alpha_step * np.sign(m_gri), Delta_Omega_container[l,m] + Omega_grid[m])
            # alpha_step *= tau_para
        else:
            alpha_step *= tau_para
...
plt.figure(figsize=(16,4),dpi = 300)
for m in range(M):
    plt.axvline(varphi_offset[m], linestyle = '--')
plt.plot(x_list, M_varphi_plots)
