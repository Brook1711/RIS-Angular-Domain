...
def prime_f_l(YK = YK, Mu = E_step_instance.Mu, kappa = 100, omega_l = 0, l = 0 ,Delta_varphi = Delta_varphi, Sigma = M_step_instance.Sigma):
    ...
    for t in range(tau):
        ...
        F_2 = np.mat(np.zeros((1,M)) , dtype = np.complex128)
        for m in range(M):
            F_2 += (Phi_H[t,m] * (1j * 2 * np.pi * m) * np.exp(1j * 2 * np.pi * m * omega_l) * (U_M.H)[m,:] * D_M)
        for k in range(K):
            c_2 = YK[k][t+l*tau].H[0,0]
            F_3 = F_1.H * F_2 
            PD_omega_l += (c_2 * F_2 * Mu[k])[0,0]  -(Mu[k].H * F_3 * Mu[k])[0,0]

    step_value = np.real(PD_omega_l)
    return step_value

x = np.linspace(-0.49, 0.49, 200)
PD_F_L_list = []

np.random.seed(10)
# Delta_varphi_random = np.random.uniform(-0.015,0.015, M)
...
np.where(varphi_plt == np.max(varphi_plt))[0][0]
